...
from vit_symm_model import StochasticSymmetryProjectedHoneycombViT, normalise_symmetries
...
# ---------- partial/stochastic output symmetrization ----------
N_SYMMETRY_SAMPLES = int(os.environ.get("N_SYMMETRY_SAMPLES", "4"))
SYMMETRY_OFFSET = int(os.environ.get("SYMMETRY_OFFSET", "0"))
...
def build_model():
    return StochasticSymmetryProjectedHoneycombViT(
        embed_dim=EMBED_DIM,
        num_heads=NUM_HEADS,
        ...
        symmetries=normalise_symmetries(_symm_group),
        permutation=tuple(range(graph.n_nodes)),
        num_symmetry_samples=N_SYMMETRY_SAMPLES,
...
plt.ylabel("Energy")
plt.title(f"{NUM_SITES}-site Gamma NIR (stochastic_symmproj)")
plt.tight_layout()
...
summary = {
    ...
    "target_model": "StochasticSymmetryProjectedHoneycombViT",
    "symmetry_model_label": "stochastic_symmproj",
    "num_symmetry_samples": int(N_SYMMETRY_SAMPLES),
    "symmetry_offset": int(SYMMETRY_OFFSET),
