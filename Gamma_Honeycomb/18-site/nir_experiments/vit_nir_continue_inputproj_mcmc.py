...
# USER OPTIONS
# ============================================================
SOURCE_SUMMARY_FILE = os.environ.get("NIR_CONTINUE_SOURCE_SUMMARY", "").strip()

USE_TRAINING_STATE = True
USE_BEST_CHECKPOINT = False
CONTINUE_ITERS = 1000
CONTINUE_TAG = "mcmc_refinement_from_nir_summary"
CONTINUE_NUM_SAMPLES = 2**12

CONTINUE_LR = 2e-3

# ---------- NQS target update ----------
TARGET_PRECONDITIONER = "minsr"
TARGET_SR_DIAG_SHIFT = 5e-6
...
LEARN_PHASE = bool(base_summary.get("learn_phase_stage_5", base_summary.get("learn_phase", True)))
...
if training_state_path is not None:
    raw_state = load_continue_state(training_state_path)
    vstate.variables = serialization.from_state_dict(vstate.variables, raw_state["variables"])
    initial_target_opt_state = raw_state.get("target_opt_state")
    initial_target_update_state = raw_state.get("target_update_state")
    initial_rng = raw_state.get("rng")
    start_iteration = int(raw_state.get("completed_iterations", 0))
    initial_best_energy = raw_state.get("best_energy")
    initial_best_energy_distance = raw_state.get("best_energy_distance_to_exact")
    initial_best_iteration = raw_state.get("best_iteration")
    ...
    start_iteration = len(collect_previous_energy(source_summary, summary_path=source_summary_path))
    initial_best_energy = source_summary.get("best_energy_seen", base_summary.get("best_energy_seen"))
...
out_prefix = run_path(
    f"out_continue_{NUM_SITES}-site_{NUM_LAYERS}_layers_{NUM_HEADS}_heads_{PATCH_SIZE}_patches"
)
...
plt.ylabel("Energy")
plt.title(f"{NUM_SITES}-site Gamma (input-projected MCMC continuation)")
plt.tight_layout()
