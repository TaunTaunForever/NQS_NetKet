...
NUM_ITERS_SR = 10000       # final PT-SR refinement
...
JOB_BASE = (
    ...
    f"{TODAY}_Gamma_ViT_SRonly_bond_token_readout"
)
...
# ============================================================
# This model currently assumes PATCH_SIZE=1, so each token is one site.
# relation_matrix is used inside attention; edge_indices/edge_relations are
# used by the final bond-token readout.
if PATCH_SIZE != 1:
    raise ValueError("Bond-token readout script currently requires PATCH_SIZE=1.")
...
plt.ylabel("Energy")
plt.title(f"{NUM_SITES}-site Gamma bond-token readout ViT (Amplitude-only SR → SR → PT-SR)")
plt.tight_layout()
