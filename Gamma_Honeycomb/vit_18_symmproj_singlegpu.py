...
# Single-GPU launcher for cluster jobs. If the scheduler already exposes only
# one device this is harmless; otherwise it keeps JAX on the first visible GPU.
...
SITE18_DIR = THIS_DIR / "18-site"
if str(SITE18_DIR) not in sys.path:
    sys.path.insert(0, str(SITE18_DIR))
...
# Pure Gamma by default. Set K_COUPLING nonzero for a Kitaev-Gamma run.
K_COUPLING = 0.0
GAMMA_COUPLING = 1.0
...
JOB_BASE = (
    f"{NUM_SITES}-site_"
    f"K_{K_COUPLING:g}_"
    f"Gamma_{GAMMA_COUPLING:g}_"
    f"{NUM_LAYERS}_layers_"
    f"{NUM_HEADS}_heads_"
    f"{PATCH_SIZE}_patches_"
    f"{NUM_SAMPLES}_samples_"
    f"{TODAY}_Gamma_ViT_SRonly_symmproj_singlegpu"
)
...
# Hamiltonian + lattice
# ============================================================
def gamma_hamiltonian(num_sites, *, k_coupling, gamma_coupling):
    print(f"Defining {num_sites}-site Gamma Honeycomb lattice from scratch")
    print("___________________________________________________")

    if num_sites != 18:
        raise ValueError("This launcher is currently configured for the 18-site [3,3] lattice.")
    ...
    kx = ky = kz = k_coupling
    gx = gy = gz = gamma_coupling

    hamiltonian = nk.operator.LocalOperator(hilbert, dtype=np.complex128)
    symm_group = graph.automorphisms()

    print("Honeycomb lattice:", extent_str)
    print("Sites:", graph.n_nodes)
    print("Edges:", graph.n_edges)
    print("Point-group symmetries:", len(symm_group))
    print("K coupling:", k_coupling)
    print("Gamma coupling:", gamma_coupling)
    print()

    for i, j, color in graph.edges(return_color=True):
        if color == 0:  # z bond
            hamiltonian += kz * nk.operator.LocalOperator(hilbert, SzSz, acting_on=[i, j])
            hamiltonian += gz * nk.operator.LocalOperator(
                hilbert, SySx_SxSy, acting_on=[i, j]
            )
        elif color == 1:  # x bond
            hamiltonian += kx * nk.operator.LocalOperator(hilbert, SxSx, acting_on=[i, j])
            hamiltonian += gx * nk.operator.LocalOperator(
                hilbert, SySz_SzSy, acting_on=[i, j]
            )
        elif color == 2:  # y bond
            hamiltonian += ky * nk.operator.LocalOperator(hilbert, SySy, acting_on=[i, j])
            hamiltonian += gy * nk.operator.LocalOperator(
                hilbert, SzSx_SxSz, acting_on=[i, j]
            )

    return graph, symm_group, hilbert, hamiltonian


graph, symm_group, hi, ha = gamma_hamiltonian(
    NUM_SITES,
...
plt.ylabel("Energy")
plt.title(f"{NUM_SITES}-site Gamma (Amplitude-only SR -> SR -> PT-SR)")
plt.tight_layout()
