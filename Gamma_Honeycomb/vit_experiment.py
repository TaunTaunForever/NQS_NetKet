...
ADAM_LR_FINAL = 5e-3                # end of Adam phase
...
NUM_SITES = 18
NUM_SAMPLES_ADAM = 3 * 2**9
NUM_SAMPLES_SR = 3 * 2**10
NUM_ITERS = 5000
...
def make_metropolis_local(hilbert, n_samples):
    ...
    n_chains = 512
    return nk.sampler.MetropolisLocal(hilbert=hilbert, n_chains=n_chains, reset_chains=True)

def make_parallel_tempering_local(hilbert, n_samples):
    n_chains = max(1, n_samples // 8)
    n_chains = 512
    return nk.sampler.ParallelTemperingLocal(hilbert=hilbert, n_chains=n_chains, reset_chains=True)
...
sampler_adam = make_metropolis_local(hi, NUM_SAMPLES_ADAM)

vstate_adam = nk.vqs.MCState(
    sampler=sampler_adam,
    model=model,
    n_samples=NUM_SAMPLES_ADAM,
...
sampler_sr = make_parallel_tempering_local(hi, NUM_SAMPLES_SR)

vstate_sr = nk.vqs.MCState(
    sampler=sampler_sr,
    model=model,
    n_samples=NUM_SAMPLES_SR,
