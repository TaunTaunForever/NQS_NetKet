...
class AutoregressiveProposalNet(nn.Module):
    ...
    def __call__(self, sigma):
        ...
        tok_emb = nn.Embed(num_embeddings=2, features=self.embed_dim, dtype=jnp.float64, param_dtype=jnp.float64)(shifted)
        bos_emb = self.param("bos_emb", nn.initializers.normal(stddev=0.02), (self.embed_dim,), jnp.float64)
        bos = jnp.broadcast_to(bos_emb[None, None, :], (sigma.shape[0], 1, self.embed_dim))
        x = jnp.concatenate([bos, tok_emb], axis=1)
        pos_emb = self.param("pos_emb", nn.initializers.normal(stddev=0.02), (self.n_sites, self.embed_dim), jnp.float64)
        x = x + pos_emb[None, :, :]
        ...
        return nn.Dense(2, dtype=jnp.float64, param_dtype=jnp.float64)(x)
