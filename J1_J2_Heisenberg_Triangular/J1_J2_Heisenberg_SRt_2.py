...
vstate_1 = nk.vqs.MCState(sampler=sa, model=ma, n_samples=2**9, chunk_size=CHUNK_SIZE)
...
# Second Run
op_1 = nk.optimizer.Sgd(learning_rate=1e-2)
vstate_2 = nk.vqs.MCState(sampler=sa, model=ma, n_samples=2**9, variables = vstate_1.variables, chunk_size=CHUNK_SIZE)
gs = nkx.driver.VMC_SRt(hamiltonian=ha, optimizer=op_1, variational_state=vstate_2, diag_shift=0.001)

gs.run(n_iter=1000, out='out_{}_1'.format(JOB_NUMBER))
...
gs.run(n_iter=NUM_ITERS, out='out_{}_2'.format(JOB_NUMBER))


#Get data from log and 
energy = []
data=json.load(open("out_{}_1.log".format(JOB_NUMBER)))
for en in data["Energy"]["Mean"]["real"]:
    # print(en)
    energy.append(en)
data=json.load(open("out_{}_2.log".format(JOB_NUMBER)))
for en in data["Energy"]["Mean"]["real"]:
    # print(en)
    energy.append(en)
