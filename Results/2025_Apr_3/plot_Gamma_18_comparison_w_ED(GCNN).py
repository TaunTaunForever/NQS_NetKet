...
samples = pd.read_table("mean_energy_run_G=1.0_18-site_5_GCNN_layers_18_width_1024_samples_2025-04-02_minSR.txt", header=None)
samples['x'] = np.arange(0,len(samples[0]), 1)
samples[0] -= -25.632707769644625
new_samples = samples[-1000:]
fig,axes = plt.subplots(1,1)
axes.set_xscale('log')
plt.axhline(y = 0, color = 'k', linestyle = '-')
plt.plot(new_samples['x'], new_samples[0])
print(new_samples['x'])
plt.xlim(new_samples['x'].to_numpy()[0], new_samples['x'].to_numpy()[-1])
plt.xlabel("Iterations")
plt.ylabel(r"$E_{GS} - E_{ED}$")
#plt.title("50-site GSE For Gamma Heisenberg Honeycomb Model \nusing Kernel Formalation of SGD+SR")



x1, x2, y1, y2 = 99000, 100000, -25.66, -25.61  # subregion of the original image
