...
samples_1 = pd.read_table("mean_energy_run_K=-1.0_24-site_4_blocks_24_width_1024_samples_2025-06-10_minSR.txt", header=None)
samples_1['x'] = np.arange(0,len(samples_1[0]), 1)

samples_2 = pd.read_table("mean_energy_run_K=-1.0_24-site_4_blocks_24_width_4096_samples_2025-06-16_minSR.txt", header=None)
samples_2['x'] = np.arange(0,len(samples_2[0]), 1)


samples_3 = pd.read_table("mean_energy_run_K=-1.0_24-site_4_blocks_24_width_8192_samples_2025-06-16_minSR.txt", header=None)
...
plt.plot(samples_1['x'][:5000], samples_1[0][:5000]/4, label="1024")
plt.plot(samples_2['x'][:5000], samples_2[0][:5000]/4, label="4096")
plt.plot(samples_3['x'][:5000], samples_3[0][:5000]/4, label="8192")
...
'''
plt.axhline(y = -19.0571934348/4, color = 'r', linestyle = '-')
plt.xlabel("Iterations")
plt.ylabel("Variational Ground State Energy")
plt.title("Ground State Energies for 24-site Kitaev Honeycomb Model")
plt.legend()
plt.show()
