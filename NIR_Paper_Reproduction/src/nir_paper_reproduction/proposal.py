...
class AutoregressiveProposalNet(nn.Module):
    ...
    def __call__(self, sigma):
        mlp_hidden_dim = (
            2 * self.embed_dim if self.mlp_hidden_dim is None else self.mlp_hidden_dim
        )
        ...
        bos = jnp.broadcast_to(
            bos_emb[None, None, :],
            (sigma.shape[0], 1, self.embed_dim),
        )
        ...
        return nn.Dense(2, dtype=PROPOSAL_DTYPE, param_dtype=PROPOSAL_DTYPE)(x)
...
def sample_from_proposal(model, params, rng, n_samples, n_sites, *, prob_floor):
    ...
    def sample_site(carry, site):
        ...
        sigma = sigma.at[:, site].set(tokens_to_spins(token))
        return (sigma, token, updated["cache"], rng), None
...
def train_proposal_step(model, params, opt_state, optimizer, samples, *, prob_floor):
    loss, grads = jax.value_and_grad(
        lambda p: forward_kl_loss(model, p, samples, prob_floor=prob_floor)
    )(params)
