...
NUM_ITERS_CONTINUE = 2000
CONTINUE_DIAGSHIFT = 2e-4
CONTINUE_LR = 1e-3
CONTINUE_MOMENTUM = 0.0
CONTINUE_TAG = "sr_continue"
RUN_SAMPLER_MODES = ("local", "pt")
SOURCE_SAMPLER_MODE = None

N_DISCARD_PER_CHAIN = 8
TARGET_CHAIN_LENGTH = 32
PT_SWEEP_SIZE_FACTOR = 3
...
continue_run_dir = make_continuation_dir(source_run_dir, "continuation_base")
...
SR_SOLVER = solver_from_name(base_summary.get("sr_solver_name", "pinv_smooth"))
CHUNK_SIZE = 2**6
CHUNK_SIZE_BWD = 2**6
...
for mode in SAMPLER_MODES:
    ...
    plt.ylabel("Energy")
    plt.title(f"{NUM_SITES}-site Kitaev continuation (base ViT, {mode})")
    plt.tight_layout()
...
continuation_summary = {
    ...
    "sr_solver_name": base_summary.get("sr_solver_name", "pinv_smooth"),
