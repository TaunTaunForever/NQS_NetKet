...
TRAIN_LR_STAGE_2 = 3e-3
TRAIN_LR_STAGE_3 = 3e-4
...
NIR_PROPOSAL_LR_STAGE_2 = 3e-4
NIR_PROPOSAL_LR_STAGE_3 = 3e-5
...
TARGET_SR_DIAG_SHIFT_STAGE_2 = 3e-4
...
JOB_BASE = (
    ...
    f"{TODAY}_Kitaev_ViT_NIR_symmproj"
)
...
symmetry_perms = tuple(symmetry_inverse_perm(g, graph.n_nodes) for g in symm_group)
...
def rebuild_vstate_with_phase(vstate, learn_phase):
    ...
    if hasattr(vstate, "_samples") and getattr(vstate, "_samples", None) is not None:
...
def run_nir_stage(
    *,
    ...
    for it in range(n_iter):
        ...
        if learn_phase != current_phase:
            vstate = rebuild_vstate_with_phase(vstate, learn_phase)
            params = vstate.parameters
...
plt.ylabel("Energy")
plt.title(f"{NUM_SITES}-site Kitaev (NIR training)")
plt.tight_layout()
