...
NUM_SAMPLES_STAGE_1 = 2**10
NUM_SAMPLES_STAGE_2 = NUM_SAMPLES_STAGE_1
NUM_SAMPLES_STAGE_3 = 2**11
...
JOB_BASE = (
    ...
    f"{TODAY}_Gamma_ViT_NIR_bondaware_multiphase_paperNIR_srt"
)
...
#   4 = non-neighbor
#
# For PATCH_SIZE>1, convert the site-level relation matrix into a patch-level
# relation matrix, so the attention bias shape matches the number of ViT tokens.
...
plt.ylabel("Energy")
plt.title(f"{NUM_SITES}-site Gamma (NIR training, bond-aware ViT)")
plt.tight_layout()
...
summary = {
    ...
    "proposal_max_batches": NIR_MAX_PROPOSAL_BATCHES,
    "proposal_max_adaptive_rounds": NIR_MAX_ADAPTIVE_ROUNDS,
    "proposal_ess_threshold_frac": NIR_ESS_THRESHOLD_FRAC,
    "nir_strategy": "paper_inspired",
    "target_preconditioner": TARGET_PRECONDITIONER,
    "target_optimizer": "srt",
