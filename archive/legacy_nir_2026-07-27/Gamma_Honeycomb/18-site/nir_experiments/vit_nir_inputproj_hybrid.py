...
TRAIN_LR_STAGE_3 = 2e-3
TRAIN_LR_STAGE_1_ITERS = 50
TRAIN_LR_STAGE_2_ITERS = 100
LOG_STEP_SIZE = 1
WRITE_EVERY = 1
SAVE_PARAMS_EVERY = 25

# ---------- Hybrid handoff ----------
# Stage 3 is the final refinement stage, so we switch to plain local-spin MCMC there.
HYBRID_MCMC_START_STAGE = 3
HYBRID_MCMC_NUM_SAMPLES = NUM_SAMPLES_STAGE_3
...
NIR_EFFICIENCY_THRESHOLD_STAGE_3 = 0.25
NIR_PROPOSAL_LR_STAGE_1 = 3e-3
NIR_PROPOSAL_LR_STAGE_2 = 3e-3
NIR_PROPOSAL_LR_STAGE_3 = 1e-4
...
TARGET_SR_DIAG_SHIFT_STAGE_3 = 5e-6
...
JOB_BASE = (
    ...
    f"{TODAY}_Gamma_ViT_NIR_inputproj_hybrid"
)
...
def current_sampling_mode(step):
    return "mcmc" if current_stage(step) >= HYBRID_MCMC_START_STAGE else "nir"
...
def run_hybrid_stage(
    *,
    ...
    current_sampling = current_sampling_mode(0)

    for it in range(n_iter):
        learn_phase = current_learn_phase(it)
        n_samples = current_num_samples(it)
        sampling_mode = current_sampling_mode(it)
        if (
            learn_phase != current_phase
            or n_samples != current_sample_count
            or sampling_mode != current_sampling
        ):
            vstate = rebuild_vstate_for_stage(vstate, learn_phase, n_samples)
            params = vstate.parameters
            current_phase = learn_phase
            current_sample_count = n_samples
            current_sampling = sampling_mode
            if sampling_mode == "mcmc":
                clear_cached_samples(vstate)

        if sampling_mode == "nir":
        ...
        history.append(
            {
                "iteration": it + 1,
                "stage": int(current_stage(it)),
                "sampling_mode": current_sampling,
                "energy": energy,
        ...
        logger_payload = {
            ...
            "Hybrid": {
                "Stage": int(current_stage(it)),
                "SamplingModeMCMC": int(current_sampling == "mcmc"),
        ...
        logger(it, logger_payload, variational_state=vstate)
        if (it + 1) % max(1, LOG_STEP_SIZE) == 0:
            if last_round is None:
                print(
                    f"it={it + 1:5d} "
                    f"Stage={current_stage(it)} "
                    f"Mode=MCMC "
                    f"Energy={energy:.8f} "
...
print(f"\n=== Hybrid training: {NUM_ITERS_TOTAL} iterations ===\n")
...
out_prefix = run_path(f"out_{JOB_BASE}_hybrid")
result = run_hybrid_stage(
...
print("Final hybrid energy:", final_energy)
print("Final hybrid gap to exact:", final_energy - EXACT_GROUND_STATE_ENERGY)
print("Best hybrid energy:", float(result["best_energy"]))
print("Best hybrid abs distance to exact:", float(result["best_energy_distance_to_exact"]))
print("Best hybrid iteration:", int(result["best_iteration"]))
...
plt.ylabel("Energy")
plt.title(f"{NUM_SITES}-site Gamma (hybrid NIR -> MCMC, input-projected)")
plt.tight_layout()
...
summary = {
    ...
    "run_dir": str(RUN_DIR),
    "num_sites": NUM_SITES,
    "hybrid_mcmc_start_stage": HYBRID_MCMC_START_STAGE,
    "hybrid_mcmc_start_iteration": TRAIN_LR_BOUNDARY_2,
    "hybrid_mcmc_num_samples": HYBRID_MCMC_NUM_SAMPLES,
...
if energy:
    ...
    summary["nir_iterations"] = int(
        sum(row["sampling_mode"] == "nir" for row in result["history"])
    )
    summary["mcmc_iterations"] = int(
        sum(row["sampling_mode"] == "mcmc" for row in result["history"])
    )
